...
const AdminNewsEvents = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setForm({ title: "", desc: "", category: "", author: "", date: "", content: "", image: null });
      setPreview(null);
      setEditingId(null);
      setIsFormOpen(false);
      fetchNews();
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-2 font-medium">Manage all news and event updates.</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            { label: "Total News/Events", value: news.length },
            { label: "Categories", value: new Set(news.map((b) => b.category)).size },
            { label: "Authors", value: new Set(news.map((b) => b.author)).size },
          ].map((stat, i) => (
              ...
              >
                <div className="p-6 border-b border-slate-100 flex items-center justify-between shrink-0">
                  <h2 className="text-2xl font-black text-slate-900">
                    {editingId ? "Edit News/Event" : "Create New News/Event"}
                  </h2>
                ...
                <form onSubmit={handleSubmit} className="p-8 overflow-y-auto space-y-8">
                  {/* --- Inputs: title, author, category, date, desc --- */}
                  <div className="grid md:grid-cols-2 gap-6">
                    {["title", "author", "category", "date", "desc"].map((field) => (
                  ...
                  <div className="pt-6 border-t border-slate-100 flex justify-end gap-3">
                    ...
                    <button className="bg-[#0F75BD] text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-[#0d63a5] transition-all">
                      {editingId ? "Update News/Event" : "Publish News/Event"}
                    </button>
                  </div>
                </form>
              </motion.div>
...
/>
        ...
        {news.length === 0 && (
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-100">
            ...
            <h3 className="text-xl font-black text-slate-900">No News Yet</h3>
            <p className="text-slate-500 mt-2">Start by creating your first news update.</p>
            <button
